...
def asiento_ocupado(artista, fecha, sector, fila, columna):
    """
    Devuelve True si la butaca (fila,columna) ya fue vendida
    para ese artista–fecha–sector, leyendo usuarios.txt.
    """
    try:
        with open("usuarios.txt", "rt", encoding="utf-8") as f:
            for linea in f:
                ...
                for s in partes[2].split(","):
                    campos = s.split("-")
                    # formato esperado: Artista-fecha-sector-fila-col
                    if len(campos) == 5:
                        a, fe, se, fi, co = campos
                        if (a == artista and fe == fecha and 
                            se == sector and fi == str(fila) and 
                            co == str(columna)):
                            return True
        ...
        with open("usuarios.txt", "r", encoding="utf-8") as archivo:
            ...
            for linea in archivo:
                ...
                if datos[0] == dni_usuario:
                    ...
                    print("Shows comprados:")
                    shows = datos[2].split(',') if len(datos) > 2 and datos[2] else []
                    for show in shows:
                        partes = show.split('-')
                        if len(partes) == 4:
                            ...
                            artista, fecha, sector, fila, columna = partes
                            print(f"- {artista} | Fecha: {fecha} | Sector: {sector} | Fila: {fila} | Columna: {columna}")
                        else:
            ...
            for linea in f:
                ...
                for s in partes[2].split(","):
                    if not s:
                        continue
                    campos = s.split("-")
                    # Agrupado: Artista–Fecha–Sector–Cantidad
                    if len(campos) == 4:
                        art, fecha, sector, cnt = campos
                        try:
                            cantidad = int(cnt)
                        except ValueError:
                            continue
                    # Butaca suelta: Artista–Fecha–Sector–Fila–Columna
                    elif len(campos) == 5:
                        art, fecha, sector, fila, col = campos
                        cantidad = 1
                    else:
                        continue
...
def reporte_ventas_por_evento():
    ...
    if not ventas:
        ...
        for evento_key, total in ventas.items():
            print(f" - {evento_key}: {total} entradas vendidas")
    print()
...
def proceder_con_compra(artista_id):
    """Permite al usuario proceder con la compra de entradas (con simulación de pago y selección de butacas)."""
    global dni_usuario
    ...
    print(f"\nDisponibilidad total en {sector}: {dispo} entradas")
    print(f"Precio por butaca: ${precio}")
    ...
    for n in range(1, cantidad+1):
        print(f"\nButaca #{n}:")
        while True:
            fila = input("  Fila (número): ")
            col  = input("  Columna (número): ")
            if not (fila.isdigit() and col.isdigit()):
                print("  Formato inválido, ingrese números.")
                continue
            fila, col = int(fila), int(col)
            # comprobamos si ya está vendida o ya la seleccionaste
            if asiento_ocupado(evento['Artista'], fecha_seleccionada, sector, fila, col) \
               or (fila, col) in butacas:
                print("  ❌ Esa butaca ya está ocupada o ya la elegiste, elige otra.")
            else:
                butacas.append((fila, col))
                break
    ...
    try:
        ...
        for line in lines:
            ...
            if dni == dni_usuario:
                ...
                # añadimos cada butaca como entrada individual
                for fila, col in butacas:
                    shows.append(f"{evento['Artista']}-{fecha_seleccionada}-{sector}-{fila}-{col}")
